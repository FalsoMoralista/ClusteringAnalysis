...
# TODO:
# (1) - Replace ReLU by LeakyReLU - ok
# (2) - Squeeze data in the loading process
# (3) - Implement Distributed Data Parallel to speedup training. 
# (4) - Logs.
...
class ExtractedFeaturesDataset(Dataset):
    def __init__(self, data_dir='../ViT/feature_extraction/features/ImageNET/imagenet_features_dictionary.pt', transform=None, target_transform=None):
...
for epoch in range(epochs):
    ...
    for (image, label) in loader:
      ...
      iteration += 1
    print('Epoch ['+ str(epoch) + '/'+ str(epochs) + '] - Loss: ', loss.item()) # format
    #outputs.append((epochs, image, reconstructed))
 
torch.save(model.state_dict(), 'denoising_autoencoder_checkpoint-epoch-{}.pth'.format(epochs))
...
for iteration in range(len(losses)):
    if iteration % total_steps == 0:
        ...
        sum = []
    sum.append(losses[iteration].numpy())

plt.plot(epoch_loss)
plt.savefig('denoising_AE_loss_x_epoch.pdf')
plt.plot(losses)
plt.savefig('denoising_AE_loss_x_iterations.pdf')